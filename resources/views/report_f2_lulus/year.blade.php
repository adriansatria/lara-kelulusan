		...
		<div class="card">
				...
				<div class="row">
					<div class="col-12">
						...
						<div class="float-right ml-2">
							@foreach($report_f2s_lulus->take(1) as $f2)
							<form class="form-inline" action="{{ route('f2s_lulus.export') }}" method="POST">
								@csrf
								<div class="form-group">
			...
			<div class="card-body">
				<table id="example1" class="table table-bordered table-striped display nowrap" width="100%">
					...
					<tbody>
						@foreach($report_f2s_lulus as $report_f2_lulus)
						<tr>
							<td>{{$loop->iteration}}</td>
							<td>{{ $report_f2_lulus->nama_mahasiswa }}</td>
							<td>{{ $report_f2_lulus->nim }}</td>
							<td>{{ $report_f2_lulus->status }}</td>
							<td>{{ $report_f2_lulus->tahun }}</td>
						</tr>
						@endforeach
					</tbody>
				</table>
