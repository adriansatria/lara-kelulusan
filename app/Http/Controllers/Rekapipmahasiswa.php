...
class Rekapipmahasiswa extends Controller
{
	...
	public function year(Request $request)
	{
		if($request->input('year') == ''){
			...
			$result = Rekapipmahasiswa_model::where('tahun', $year)->get();

			$ip1_max = Rekapipmahasiswa_model::where('tahun', $year)->max('ip_s1');
			$ip1_min = Rekapipmahasiswa_model::where('tahun', $year)->min('ip_s1');
			$ip1_avg = number_format(Rekapipmahasiswa_model::where('tahun', $year)->avg('ip_s1'), 2);

			$ip1_max = Rekapipmahasiswa_model::where('tahun', $year)->max('ip_s1');
			$ip1_min = Rekapipmahasiswa_model::where('tahun', $year)->min('ip_s1');
			$ip1_avg = number_format(Rekapipmahasiswa_model::where('tahun', $year)->avg('ip_s1'), 2);

			$ip2_max = Rekapipmahasiswa_model::where('tahun', $year)->max('ip_s2');
			$ip2_min = Rekapipmahasiswa_model::where('tahun', $year)->min('ip_s2');
			$ip2_avg = number_format(Rekapipmahasiswa_model::where('tahun', $year)->avg('ip_s2'), 2);

			$ip3_max = Rekapipmahasiswa_model::where('tahun', $year)->max('ip_s3');
			$ip3_min = Rekapipmahasiswa_model::where('tahun', $year)->min('ip_s3');
			$ip3_avg = number_format(Rekapipmahasiswa_model::where('tahun', $year)->avg('ip_s3'), 2);

			$ip4_max = Rekapipmahasiswa_model::where('tahun', $year)->max('ip_s4');
			$ip4_min = Rekapipmahasiswa_model::where('tahun', $year)->min('ip_s4');
			$ip4_avg = number_format(Rekapipmahasiswa_model::where('tahun', $year)->avg('ip_s4'), 2);

			$ip5_max = Rekapipmahasiswa_model::where('tahun', $year)->max('ip_s5');
			$ip5_min = Rekapipmahasiswa_model::where('tahun', $year)->min('ip_s5');
			$ip5_avg = number_format(Rekapipmahasiswa_model::where('tahun', $year)->avg('ip_s5'), 2);

			$ip6_max = Rekapipmahasiswa_model::where('tahun', $year)->max('ip_s6');
			$ip6_min = Rekapipmahasiswa_model::where('tahun', $year)->min('ip_s6');
			$ip6_avg = number_format(Rekapipmahasiswa_model::where('tahun', $year)->avg('ip_s6'), 2);

			$ip7_max = Rekapipmahasiswa_model::where('tahun', $year)->max('ip_s7');
			$ip7_min = Rekapipmahasiswa_model::where('tahun', $year)->min('ip_s7');
			$ip7_avg = number_format(Rekapipmahasiswa_model::where('tahun', $year)->avg('ip_s7'), 2);

			$ip8_max = Rekapipmahasiswa_model::where('tahun', $year)->max('ip_s8');
			$ip8_min = Rekapipmahasiswa_model::where('tahun', $year)->min('ip_s8');
			$ip8_avg = number_format(Rekapipmahasiswa_model::where('tahun', $year)->avg('ip_s8'), 2);

			$ipk_max = Rekapipmahasiswa_model::where('tahun', $year)->max('ipk');
			$ipk_min = Rekapipmahasiswa_model::where('tahun', $year)->min('ipk');
			$ipk_avg = number_format(Rekapipmahasiswa_model::where('tahun', $year)->avg('ipk'), 2);
